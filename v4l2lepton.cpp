...
static void grab_frame() {
    ...
    while( segmentNumber != 4 ){
        ...
        for (int j = 0; j < PACKETS_PER_FRAME; j++) {
            ...
            // fprintf( stderr, "resets: %d, segmentNumber: %d, packetNumber: %d\n", resets, segmentNumber, packetNumber );
        }
        if (resets >= 30) {
    ...
    for(int iSegment = 1; iSegment <= 4; iSegment++) {
        ...
        for (int i = 0; i < FRAME_SIZE_UINT16; i++) {
            ...
            value = ((shelf[iSegment - 1][i*2] << 8) + shelf[iSegment - 1][i*2+1] - minValue) * scale;
            const int *colormap = colormap_ironblack;
            column = (i % PACKET_SIZE_UINT16) - 2 + (width / 2) * ((i % (PACKET_SIZE_UINT16 * 2)) / PACKET_SIZE_UINT16);
